...
const Principal = () => {
  ...
  useEffect(() => {
    if (!filtros) {
      ...
      let tem = characters?.filter(
        (personaje) => personaje.gender === filtros
      ); //Fitran los productos por categoria
  ...
  useEffect(() => {
    if (query !== "") {
      let temp = charactersFiltrados.filter((obj) => obj.title.includes(query)); //Busca en los titulos que incluyan la palabra de la busqueda
      setBuscados(temp);
    } else {
      setBuscados(charactersFiltrados); //Seteamos el valor de los productos buscados
    }
  ...
  return (
    ...
    <div className="personajes">
      ...
      <Listado
        characters={buscados !== null ? buscados : charactersFiltrados} />
    </div>
